...
[dose-calculator/index.html](dose-calculator/index.html) is a JS rewrite of a legacy
Flutter feature that is no longer in this repo: find a medicine (typed name or scanned
barcode), enter dose / frequency / days, get the number of packs to dispense. Both pages
are reached through the **Pages** dropdown beside the title — markup sits in each page's
header, behaviour in [js/site-nav.js](js/site-nav.js).

Four docs cover it in [dose-calculator/documentations/](dose-calculator/documentations/);
start with `dose-calculator.md`.

- **Its own data file.** [dose-calculator/assets/imFile.csv](dose-calculator/assets/imFile.csv)
  is headerless with fixed columns (`IM_COL` in
  [dose-calculator/js/dose-item.js](dose-calculator/js/dose-item.js)): sku=0, name_ar=1,
  name_en=2, item_type=3, units=4, unit_type=5, dose_frequency=6, note=7, image_url=8.
  Its line endings are **bare carriage returns** — not CRLF, not LF — so the shared
  `splitLines` cannot split it at all. `imfile-store.js` therefore does its own line
  splitting and uses the shared `parseCSVLine` for fields only.
- **The maths is another verbatim port** — of the Dart `DoseCalculationService`, itself a
  port of an older C#. The pack-size substitutions (28→30, 56→60, 14→15, 7→7.5), the inert
  `+ 0.01` rounding nudge, and the fact that antibiotics take the Bupa round-to-nearest
  branch are all deliberate; preserve them. The legacy Dart and C# are gone from the repo
  but reproduced verbatim in the appendix of
  [legacy-calculation-rules.md](dose-calculator/documentations/legacy-calculation-rules.md),
  which is the ground truth to check a port question against. What the rewrite kept and
  what it deliberately did not fix is in
  [dose-calculation-rules.md](dose-calculator/documentations/dose-calculation-rules.md).
- **Barcode search borrows `items.csv`.** imFile.csv has no barcode column, so a scan is
  resolved barcode → SKU through the master, using the same worker and the same
  `lookupRowIndex` as the main page. The master reaches this page as a
  `FileSystemFileHandle` parked in IndexedDB by `onPickMasterFile`
  ([js/master-handle-store.js](js/master-handle-store.js)) — the file itself is never
  copied or cached. See
  [barcode-to-sku.md](dose-calculator/documentations/barcode-to-sku.md).
- **One search box, one card.** Typing opens a ranked suggestion dropdown; picking an item
  reveals the Calculation details card; Calculate puts a bare pack count in the box beside
  the item name, with no breakdown. That ranking is the one intentional deviation from the
  legacy port — the match set is still the old `LIKE '%q%'` on `name_en`, but hits are
  ordered closest-first instead of file order.

## Editing conventions

- Keep the ported logic (barcode length rules, print shell, dose maths) faithful to its
  Dart origin rather than "cleaning it up" — matching the legacy system's output is the
  point.
